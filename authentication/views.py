...
#thread allows us to run different processes in one program
...
def send_activation_email(user, request):
    ...
    # force_bytes convert id interger into byte format
    ...
    #we send the email only when we are not testing
...
def register(request):
    ...
    if request.method == "POST":
        ...
        # validate email using a package called validate_email (can install using pip install validate_email)
        ...
        # checking if username is already used in db
        ...
        # checking if Email is already used in db
...
def login_user(request):
    ...
    if request.method == 'POST':
        ...
        # authenticate the user using its credentials
        user = authenticate(request, username=username, password=password)

        # if we didn't see the user
